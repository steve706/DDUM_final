...
from fusion_block import DF_Module
...
#                                                               norm_layer=norm_layer) for i in range(2)])
       ...
       # self.Conv1_1= nn.Conv2d(256, 512, kernel_size=3,dilation=2,padding="same")
        ...
        #
        # pidinet = PiDiNet(30, config_model('carv4'), dil=12, sa=True).eval()
        #
        # # load weights
        # checkpoint_PDC = torch.load('./weights/table5_pidinet-small.pth')
...
#######################################################################
class SRF_UNet_final(nn.Module):
    def __init__(self, num_heads, window_size, n=1,
                 img_ch=3,
                 output_ch=1,
                 attention=False,
                 ...
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm):
        super(SRF_UNet_final, self).__init__()
        filters = [64, 128, 256, 512]

        self.block_4 = nn.ModuleList([SwinTransformerBlock(dim=1024, input_resolution=(20, 20),
                                                           num_heads=32, window_size=4,
                                                           shift_size=0 if (i % 2 == 0) else window_size // 2,
                                                           mlp_ratio=mlp_ratio,
                                                           qkv_bias=qkv_bias, qk_scale=qk_scale,
                                                           ...
                                                           drop_path=drop_path[i] if isinstance(drop_path,
                                                                                                list) else drop_path,
                                                           norm_layer=norm_layer) for i in range(2)])
        self.block_3 = nn.ModuleList([SwinTransformerBlock(dim=512, input_resolution=(38, 38),
                                                           num_heads=16, window_size=19,
                                                           shift_size=0 if (i % 2 == 0) else window_size // 2,
                                                           mlp_ratio=mlp_ratio,
                                                           qkv_bias=qkv_bias, qk_scale=qk_scale,
       ...
       # self.eff=EFF(2,1)
        ...
        self.maxpool_3 = nn.MaxPool2d(2)
        self.maxpool_4 = nn.MaxPool2d(2)
        self.NAFFA=NA_FFA(in_channels=1)
        self.gate_fusion = DepthAwareGatingWithSpatialAttention(in_channels=32)

    def forward(self, x,thick,DVC):
        x_=self.NAFFA(thick,DVC,x)
        # encoding path
        # x = self.dep(x)
