...
if __name__=='__main__':
   ...
   # device="cpu"
    ...
    sub_dir = "rose-all" + "/second_stage_v2"  # + args.model + "/" + args.loss
    ...
    if isTraining:  # train
        ...
        second_net = SRF_UNet_final(img_ch=args.input_nc, output_ch=1, attention=True, num_heads=3, window_size=7).to(device)
        first_net_thick = torch.load(
            args.models_dir + "/" + "rose-svc"+ "/first_stage/front_model-" + args.first_suffix).to(
        ...
        first_net_thin = torch.load(
            args.models_dir + "/" + "rose-dvc" + "/first_stage/front_model-" + args.first_suffix1).to(
            device)  # two stage时可以加载first_stage和second_stage的模型
        first_net_thin = first_net_thin.module
        first_net_thin.eval()
        # second_net = fusion(channels=args.base_channels, pn_size=args.pn_size, kernel_size=3, avg=0.0, std=0.1).to(
        #    device)  # ##
        #second_net = FR_UNet(num_channels=1).to(
         #   device)
        ...
        thick_criterion = build_loss("dice_fusion") # 可更改
        thin_criterion = build_loss("dice_fusion")# 可更改
        fusion_criterion =build_loss("dice_fusion")#torch.nn.BCELoss()#nn.BCELoss()#build_loss("dice_fusion") # 可更改
        #fusion_criterion =torch.nn.CrossEntropyLoss()        #fusion_criterion =torch.nn.CrossEntropyLoss()


        best_fusion = {"epoch": 217, "auc":0.86,"acc":0,"dice":0}
        # start training
        print("Start training...")
        for epoch in range(args.second_epochs):
            #print('Epoch %d / %d' % (epoch + 218, args.second_epochs + 217))
            print('Epoch %d / %d' % (epoch , args.second_epochs ))
            print('-' * 10)
            second_net = train_second_stage(viz, writer, train_dataloader, first_net_thick, first_net_thin, second_net,
                                            second_optim, args.init_lr, thick_criterion,fusion_criterion, device, args.power,
                                            epoch, args.second_epochs)#+300
        ...
        test_second_stage(val_dataloader, first_net, first_net1, second_net, device, args.results_dir + "/" + sub_dir,
