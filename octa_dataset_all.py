...
class ROSE(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.channel == 1:
            ...
            # deep= deep.convert("RGB")
            # superficial = superficial.convert("RGB")
        T=transforms.CenterCrop((200,200))
        X=transforms.Pad([52, 52], fill=(0, 0, 0), padding_mode='constant')
        superficial=T(superficial)
        superficial=X(superficial)
    ...
    def get_dataPath(self, root, isTraining):
        ...
        """
        if isTraining:
            img_dir = os.path.join(root + "/SVC_DVC/train/img")
            gt_dir = os.path.join(root + "/SVC_DVC/train/gt")
            deep_dir = os.path.join(root + "/SVC/train/img")
            superficial_dir = os.path.join(root + "/DVC/train/img")
        else:
            img_dir = os.path.join(root + "/SVC_DVC/test/img/")
            gt_dir = os.path.join(root + "/SVC_DVC/test/gt/")
            deep_dir = os.path.join(root + "/SVC/test/img/")
            superficial_dir = os.path.join(root + "/DVC/test/img/")
