...
def build_dataset(dataset, data_dir, channel=1, isTraining=True, crop_size=(64, 64), scale_size=(512, 512)):
    ...
    elif dataset == "rose-dvc":
        from octa_dataset_dvc import ROSE
        database = ROSE(data_dir, channel=channel, isTraining=isTraining)
    elif dataset == "rose-svc":
        from octa_dataset_svc import ROSE
        database = ROSE(data_dir, channel=channel, isTraining=isTraining)
    elif dataset == "rose-all":
        from octa_dataset_all import ROSE
        database = ROSE(data_dir, channel=channel, isTraining=isTraining)
    elif dataset == "OCTA500_3M":
        from octa_dataset import OCTA500_3M
        database = OCTA500_3M(data_dir, channel=channel, isTraining=isTraining)
    elif dataset == "OCTA500_6M":
        from octa_dataset import OCTA500_6M
        database = OCTA500_6M(data_dir, channel=channel, isTraining=isTraining)
    elif dataset == "rose-o":
        from octa_dataset_ROSE_O import ROSE_O
        database = ROSE_O(data_dir, channel=channel, isTraining=isTraining)
    elif dataset == "prevent":
        from octa_dataset_prevent import Prevent
        database =  Prevent(data_dir, channel=channel, isTraining=isTraining)
    else:
